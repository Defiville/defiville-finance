...
async function getMetamaskWallet() {
    ...
    if (walletProvider.networkVersion == 100) {
        await balanceOfXDAI()
    }
  ...
  if (walletProvider !== undefined) {
    ...
    if (artist == 'Select artist' || artistTip == '') {
      $('.errorMessage').text('Please select an artist and a tip amount').show(1).delay(3000).hide(1);
        ...
        const radioTips = '0x529FEa06E625d8712622FCbd373Fe5EB944C9AB6'
        ...
        const tokenAddress = getXDAITokens()['XDAI']
...
async function balanceOfXDAI() {
    try {
        ...
        if (tokenBalance > 0) {
            $('#xdaiBalance').append(tokenBalance.toString().substring(0,7));
        } else {
            $('#xdaiBalance').text(0 + ' XDAI');
        }
    } catch (error) {
        console.log(error);
    }
